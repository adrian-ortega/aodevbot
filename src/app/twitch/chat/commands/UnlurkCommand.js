...
const { getTimeDifferenceForHumans, getTimeDifferenceInDays, getTimeDifferenceInMinutes, getTimeDifferenceInHours } = require("../../../support/time");
const { botMessageReply } = require("../commands");
const { USER_ID, USER_DISPLAY_NAME } = require("../state-keys");

const getStoredLurkItem = async (twitch_id) => KeyValue.findOne({
...
const getRandomReturnMessage = (state, { duration, timestamp }) => {
  const name = state[USER_DISPLAY_NAME];
  return randomFromArray(getValues([
...
exports.name = 'unlurk';
exports.description = 'Will check to see if a user has lurked in the past to welcome them back. It also shames...';
